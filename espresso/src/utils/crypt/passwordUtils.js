...
async function hashPassword(plainPassword) {
    if (!plainPassword) {
        throw ValidationError("Password is required for hashing.", UTILITY_LAYER);
    }
    try {
...
async function verifyPassword(plainPassword, hashedPassword) {
    if (!plainPassword || !hashedPassword) {
        throw ValidationError("Both plain and hashed passwords are required for verification.", UTILITY_LAYER);
    }
    try {
